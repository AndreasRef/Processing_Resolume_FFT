...
RadioButton snareClipsOrColumns;
RadioButton baseClipsOrColumns;
...
void draw() {
  ...
  for (int i = 0; i < myAudioRange; ++i) {
    ...
    if (i==snare) {
      if ((myAudioFFT.getAvg(snare) * myAudioAmp) * myAudioIndexAmp > snareThreshold && snareTimer > snareTimerThreshold && snareOscToggle==true) { //If volume exceeds threshold
        if (snareTimer > snareTimerThreshold && snareOscToggle==true) {
          if (snareClipsOrColumns.getValue() == 0) { // Trig clips (not columns)
          ...
          } else if (snareClipsOrColumns.getValue() == 1 ) { //Trig entire columns
  ...
  //println(baseOverUnder.getValue());
  ...
  //Warning text
  if (snareOscToggle && baseOscToggle && (baseClipsOrColumns.getValue() == 1) && (snareClipsOrColumns.getValue() == 1)) {
    fill(255, 0, 0);
